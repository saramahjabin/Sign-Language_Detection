...
config.gpu_options.per_process_gpu_memory_fraction = 0.5 # Change the value as per your memory requirement
...
class Application:
    def __init__(self):
             ...
             self.json_file = open("directory/model-bw.json", "r")
             self.model_json = self.json_file.read()
             self.json_file.close()
             self.loaded_model = model_from_json(self.model_json)
             self.loaded_model.load_weights("directory/model-bw.h5")

             self.json_file_dru = open("directory/model-bw_dru.json" , "r")
             self.model_json_dru = self.json_file_dru.read()
             self.json_file_dru.close()
             self.loaded_model_dru = model_from_json(self.model_json_dru)
             self.loaded_model_dru.load_weights("directory/model-bw_dru.h5")

             self.json_file_tkdi = open("directory/model-bw_tkdi.json" , "r")
             self.model_json_tkdi = self.json_file_tkdi.read()
             self.json_file_tkdi.close()
             self.loaded_model_tkdi = model_from_json(self.model_json_tkdi)
             self.loaded_model_tkdi.load_weights("directory/model-bw_tkdi.h5")

             self.json_file_smn = open("directory/model-bw_smn.json" , "r")
             self.model_json_smn = self.json_file_smn.read()
             self.json_file_smn.close()
             self.loaded_model_smn = model_from_json(self.model_json_smn)
             self.loaded_model_smn.load_weights("directory/model-bw_smn.h5")
    ...
    def action_call(self) :
        ...
        # return 
        self.photo2 = tk.PhotoImage(file='Pictures/bg.png')
        ...
        self.tx.config(text = "Conducted By", fg="red", font = ("Courier",30,"bold"))

        
        self.photo3 = tk.PhotoImage(file='Pictures/sara.png')
        self.w3 = tk.Label(self.root1, image = self.photo3)
        self.w3.place(x = 350, y = 90)
        self.tx3 = tk.Label(self.root1)
        self.tx3.place(x = 350,y = 340)
        self.tx3.config(text = "Sara Mahjabin\ID:20-42532-1", font = ("Courier",15,"bold"))
        ...
        self.tx6.place(x = 230,y = 700)
        self.tx6.config(text = "Prof. Dr. Md. Asraf Ali", font = ("Courier",30,"bold"))
